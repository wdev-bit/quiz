<?php 
    ...
    if(isset($_POST['submit'])){
        ...
        $result = mysqli_query($con,"UPDATE questions SET quiz_id='$quiz_id' , round_id='$round_id' , title='$title' , answers='$answers' , correct_answer='$correct_answer' ");
        if($result){
            header("Location: ".base_url()."admin/quiz/list_q.php");
        }
    }
    $id = $_GET['id'];
    $res = mysqli_query($con,"SELECT questions.* , rounds.display_on FROM questions inner join rounds on questions.round_id=rounds.id WHERE `questions`.`id`='$id'");
                                    ...
                                    <form action="" method="POST" enctype="multipart/form-data">
											...
											<div class="col-md-10">
                                                <select name="quiz_id" class="form-control select_quiz" required">
                                                    ...
                                                    <?php while($row = mysqli_fetch_assoc($result)): ?>
                                                        <option value="<?php echo $row['id'] ?>" <?php if($row['id'] == $data['quiz_id']){ echo 'selected="selected"'; } ?>><?php echo $row['title'] ?></option>
                                                    <?php endwhile; ?>
                                                </select>
											</div>
                                        </div>
                                        <div class="form-group row">
											...
											<div class="col-md-10">
                                                <select name="round_id" id="rounds" class="form-control" required>
                                                    ...
                                                    <?php $qid = $data['quiz_id'];  $res = mysqli_query($con,"SELECT * FROM rounds WHERE quiz_id='$qid'"); ?>
                                                    <?php $i=0; while($r = mysqli_fetch_assoc($res)): ?>
                                                        <option value="<?php echo $r['id'] ?>" <?php if($r['id'] == $data['round_id']){ echo 'selected="selected"'; } ?>><?php echo ++$i; ?></option>
                                                    <?php endwhile; ?>
                                                </select>
											</div>
                                        </div>
                                        ...
                                        <div class="form-group row">
											...
											<div class="col-md-10" id="answers">
                                                <?php $ans = json_decode($data['answers']); ?>
                                                <div class="row">
                                                    <div class="col-md-12 pt-2" style='background-color: <?php if($data["correct_answer"] == 1){echo "#00a86b"; }else{echo "#eee"; } ?>'>
                                                        <input type="radio" id="a1" name="correct_answer" <?php if($data['correct_answer'] == 1){ echo 'checked="checked"'; } ?> value="1" required>
                                                        <label for="a1">Marked as correct answer</label>
                                                        <input type="text" class="col-md-12 form-control mb-3" value="<?php echo $ans[0]; ?>" name="answers[]" required>
                                                    </div>
                                                </div>
                                                <div class="row">
                                                    <div class="col-md-12 pt-2" style='background-color: <?php if($data["correct_answer"] == 2){echo "#00a86b"; }else{echo "#eee"; } ?>'>
                                                        <input type="radio" id="a2" name="correct_answer" <?php if($data['correct_answer'] == 2){ echo 'checked="checked"'; } ?> value="2" required>
                                                        <label for="a2">Marked as correct answer</label>
                                                        <input type="text" class="col-md-12 form-control mb-3" value="<?php echo $ans[1]; ?>" name="answers[]" required>
                                                    </div>
                                                </div>
                                                <div class="row">
                                                    <div class="col-md-12 pt-2" style='background-color: <?php if($data["correct_answer"] == 3){echo "#00a86b"; }else{echo "#eee"; } ?>'>
                                                        <input type="radio" id="a3" name="correct_answer" <?php if($data['correct_answer'] == 3){ echo 'checked="checked"'; } ?> value="3" required>
                                                        <label for="a3">Marked as correct answer</label>
                                                        <input type="text" class="col-md-12 form-control mb-3" value="<?php echo $ans[2]; ?>" name="answers[]" required>
                                                    </div>
                                                </div>
                                                <div class="row">
                                                    <div class="col-md-12 pt-2" style='background-color: <?php if($data["correct_answer"] == 4){echo "#00a86b"; }else{echo "#eee"; } ?>'>
                                                        <input type="radio" id="a4" name="correct_answer" <?php if($data['correct_answer'] == 4){ echo 'checked="checked"'; } ?> value="4" required>
                                                        <label for="a4">Marked as correct answer</label>
                                                        <input type="text" class="col-md-12 form-control mb-3" value="<?php echo $ans[3]; ?>" name="answers[]" required>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                ...
                $.ajax({
                    ...
                    success: function(response){
                        ...
                        for(var i=0; i<response.length; i++){
                            round += `
                                <option value="${response[i].id}">${i+1}</option>
                            `;
                        }
                        $('#rounds').html(round);
                    }
